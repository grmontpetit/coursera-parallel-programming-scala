...
object ParallelParenthesesBalancing {
   ...
   */
  def balanceV1(chars: Array[Char]): Boolean = {
    def iterate(c: List[Char], parenthesis: List[Char]): List[Char] = {
      c match {
        case Nil         => parenthesis
        case '(' :: tail => iterate(tail, '(' :: parenthesis)
        case ')' :: tail => if (parenthesis.isEmpty) List('N') else iterate(tail, parenthesis.tail)
        case _           => iterate(c.tail, parenthesis)
      }
    }
    iterate(chars.toList, List.empty).isEmpty
  }

  /**
    * Version 2 of balance using Integers only.
    */
